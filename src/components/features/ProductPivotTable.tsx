    ...
    <>
      ...
      <div className="block md:hidden space-y-4">
        {products.map((product) => {
          const totalValue = (product.suppliers || []).reduce(
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-blue-600 font-mono text-xs font-bold mt-0.5">
                          ${prov.currentPrice.toFixed(2)}
                          <span className="text-slate-400 font-sans font-normal">/u</span>
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className="text-right">
                          <p className="text-slate-600 text-sm">
                            <span className="font-bold text-slate-800">{prov.stock}</span>{' '}
                        ...
                        {editingId === prov.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="text-white bg-green-500 px-2 py-1 rounded text-xs font-bold"
                            >
                              OK
                            </button>
                          </div>
        ...
        <table className="w-full text-left border-collapse min-w-max">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-sm font-medium">
              ...
              {allSuppliers.map((s) => (
                <th key={s} className="px-6 py-3 whitespace-nowrap min-w-[200px] text-center">
                  {s}
                </th>
              ))}
              ...
              <th className="px-6 py-3 text-right whitespace-nowrap">PVP</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {products.map((product) => {
              ...
              const precioUnitarioTotal = (product.suppliers || []).reduce(
                (sum, p) => sum + p.currentPrice,
                0
              );

              const providerCount = (product.suppliers || []).length;
              ...
              return (
                <tr key={product.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        {prov ? (
                          <div className="mx-auto flex flex-col bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm w-max min-w-[150px] max-w-[220px] hover:border-blue-300 transition-colors">
                            <div className="flex flex-col gap-1 border-b border-slate-100 pb-3 mb-3">
                              ...
                              <span className="font-mono text-blue-600 font-bold text-lg leading-none">
                                ${prov.currentPrice.toFixed(2)}
                              </span>
                            ...
                            {editingId === prov.id ? (
                              <div className="flex flex-col gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    onClick={() => handleUpdate(prov.id)}
                                    className="text-white bg-green-500 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-600 transition-colors shadow-sm"
                                  >
                                    OK
                                  </button>
                                </div>
                          ...
                          <span className="text-slate-300 italic block text-center py-4 bg-slate-50 rounded-lg border border-dashed border-slate-200">
                            No suministra
                          </span>
                        )}
                      </td>
                  ...
                  <td className="px-6 py-4 text-right whitespace-nowrap">
                    {product.totalStock < (product.minStockThreshold ?? 0) ? (
                      <div className="flex items-center justify-end gap-2 animate-pulse">
                        ...
                        <span className="px-2 py-1 rounded text-xs font-black bg-red-100 text-red-700 border border-red-200 shadow-sm">
                          {product.totalStock} uds. (CRÍTICO)
                        </span>
                      </div>
                    ) : (
                      <span
                        className={`px-2 py-1 rounded text-xs font-bold ${product.totalStock > 10 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}
                      >
                  ...
                  <td>
                    <div className="flex items-center justify-between mt-2 bg-slate-50 px-2.5 py-1.5 rounded-md border border-slate-100">
                      <span className="text-[10px] text-slate-500 font-bold uppercase">PVP</span>
                      <span className="font-bold text-slate-700 font-mono text-xs">
                        $
                        {providerCount > 0
                          ? (precioUnitarioTotal.toFixed(2) / providerCount).toFixed(2)
                          : 0}
                      </span>
                    </div>
                  </td>
