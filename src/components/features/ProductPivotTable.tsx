    ...
    <>
      ...
      <div className="block md:hidden space-y-4">
        {products.map(product => {
          ...
          return (
            <div key={product.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              {/* Header de la Tarjeta */}
              <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-start">
                <div>
                ...
                <div className="flex gap-1">
                  <button onClick={() => openModal(product)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-full transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(product.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-full transition-colors">
              ...
              <div className="px-5 py-3 flex justify-between items-center border-b border-slate-100">
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Stock Total</p>
                  <p className="text-lg font-bold text-slate-800">{product.totalStock} <span className="text-xs font-normal text-slate-500">uds.</span></p>
                </div>
              ...
              <div className="divide-y divide-slate-100">
                {(product.suppliers || []).map(prov => {
                  ...
                  return (
                    <div key={prov.id} className="p-5 flex justify-between items-center hover:bg-slate-50 transition-colors">
                      <div>
                        <p className="font-semibold text-slate-700 text-sm">{prov.supplierName}</p>
                        <p className="text-blue-600 font-mono text-xs font-bold mt-0.5">${prov.currentPrice.toFixed(2)}<span className="text-slate-400 font-sans font-normal">/u</span></p>
                      </div>
                      
                      <div className="flex flex-col items-end gap-2">
                        <div className="text-right">
                          <p className="text-slate-600 text-sm"><span className="font-bold text-slate-800">{prov.stock}</span> <span className="text-xs text-slate-400">uds.</span> | <span className="font-mono text-xs font-semibold">${provValue.toFixed(2)}</span></p>
                        </div>
                        ...
                        {editingId === prov.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <button onClick={() => handleUpdate(prov.id)} className="text-white bg-green-500 px-2 py-1 rounded text-xs font-bold">OK</button>
                          </div>
                        ) : (
                          <button 
                            onClick={() => { setEditingId(prov.id); setNewStock(prov.stock); }}
                ...
                {allSuppliers.map(supplier => {
                  ...
                  return (
                    <td key={supplier} className="px-6 py-4 text-sm text-slate-600 align-top">
                      {prov ? (
                        <div className="mx-auto flex flex-col bg-white p-3 sm:p-4 rounded-xl border border-slate-200 shadow-sm w-max min-w-[150px] max-w-[220px] hover:border-blue-300 transition-colors">
                          <div className="flex flex-col gap-1 border-b border-slate-100 pb-3 mb-3">
                            <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Precio Unitario</span>
                            <span className="font-mono text-blue-600 font-bold text-lg leading-none">${prov.currentPrice.toFixed(2)}</span>
                            
                            <div className="flex items-center justify-between mt-2 bg-slate-50 px-2.5 py-1.5 rounded-md border border-slate-100">
                               <span className="text-[10px] text-slate-500 font-bold uppercase">Val Total</span>
                               <span className="font-bold text-slate-700 font-mono text-xs">${provValue.toFixed(2)}</span>
                            </div>
                          </div>
                          
                          {editingId === prov.id ? (
                            <div className="flex flex-col gap-2">
                              <span className="text-[10px] text-blue-600 uppercase font-bold tracking-wider">Actualizar Stock</span>
                              <div className="flex items-center gap-2">
                                <input 
                                  type="number" 
                                  ...
                                  onChange={(e) => setNewStock(parseInt(e.target.value))}
                                />
                                <button onClick={() => handleUpdate(prov.id)} className="text-white bg-green-500 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-green-600 transition-colors shadow-sm">OK</button>
                              </div>
                            </div>
                          ) : (
                            <div className="flex items-center justify-between group">
                              <div className="flex flex-col">
                                ...
                                <span className="text-slate-800 font-bold text-sm">{prov.stock} <span className="text-xs font-normal text-slate-500">uds.</span></span>
                              </div>
                              <button 
                                onClick={() => { setEditingId(prov.id); setNewStock(prov.stock); }}
                      ...
                      ) : <span className="text-slate-300 italic block text-center py-4 bg-slate-50 rounded-lg border border-dashed border-slate-200">No suministra</span>}
                    </td>
                  );
                })}
